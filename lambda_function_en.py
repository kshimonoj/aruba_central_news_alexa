...
def create_message(ap_status,firmware,total_client,application,top_client):
    ...
    text = jst.strftime("Information at %Y-%m-%d, %H:%M")   
#    text = time.strftime("%Y年%m月%d日%H時%M分頃の情報です", time.gmtime())
    ...
    text = "{} APs are up, {} APs are down".format(up_count,down_count)
    message.append({
		"uid": str(uuid.uuid4()),
		"updateDate": time.strftime("%Y-%m-%dT%H:%M:%S.00Z", time.gmtime()),
        "titleText":text, 
    ...
    text = "Current Firmware version is {}, Aruba recommended version is {}".format(current,recommended)
    message.append({
		"uid": str(uuid.uuid4()),
		"updateDate": time.strftime("%Y-%m-%dT%H:%M:%S.00Z", time.gmtime()),
        "titleText":text, 
    ...
    text = "Number of connected devices are {}".format(total_client)
    message.append({
		"uid": str(uuid.uuid4()),
		"updateDate": time.strftime("%Y-%m-%dT%H:%M:%S.00Z", time.gmtime()),
        "titleText":text, 
    ...
    text = "Most application usage is {}, percentage is {}".format(name,percent)
    message.append({
		"uid": str(uuid.uuid4()),
		"updateDate": time.strftime("%Y-%m-%dT%H:%M:%S.00Z", time.gmtime()),
        "titleText":text, 
    ...
    text = "Most traffic used device is {}, Received traffic are {} bytes, received traffic are {} bytes".format(name,rx_bytes,tx_bytes)
    message.append({
		"uid": str(uuid.uuid4()),
		"updateDate": time.strftime("%Y-%m-%dT%H:%M:%S.00Z", time.gmtime()),
        "titleText":text, 
